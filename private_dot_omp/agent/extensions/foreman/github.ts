    ...
    array(
      JSON.parse(
        await run(
          ...
          [
            ...
            "all",
            "--json",
            `${prFields},baseRefName,headRepository,headRepositoryOwner`,
          ],
          cwd,
        ),
      ),
      ...
      .map((value) => {
        ...
        return {
          ...
          headRepo:
            row.headRepository === null || row.headRepositoryOwner === null
              ? ""
              : `${text(object(row.headRepositoryOwner).login)}/${text(object(row.headRepository).name)}`,
        };
      })
      .filter(
        (item) =>
