...
export function request<T = unknown>(
  ...
  if (signal?.aborted)
    return Promise.reject(
      new HerdrError(`${method} cancelled before request was sent`, "aborted"),
    );
  ...
  const abort = () =>
    finish(
      new HerdrError(
        `${method} cancelled; mutation outcome may be uncertain. Inspect the native session before retrying.`,
        "aborted",
      ),
    );
...
export async function sessions(signal?: AbortSignal): Promise<SessionEntry[]> {
...
export async function ensureSession(
  ...
  const existing = (await sessions(signal)).find((session) => session.name === name);
  signal?.throwIfAborted();
  if (existing?.running) {
    await request(existing.socket_path, "ping", {}, 5000, signal);
    signal?.throwIfAborted();
  ...
  while (Date.now() < deadline) {
    ...
    const ready = (await sessions(signal)).find(
      (session) => session.name === name && session.running,
    );
    if (ready) {
      try {
        await request(ready.socket_path, "ping", {}, 1000, signal);
        signal?.throwIfAborted();
...
export async function startWorker(
  ...
  try {
    ...
    throw new HerdrError(
      String(error),
      signal?.aborted ? "aborted" : error instanceof HerdrError ? error.code : undefined,
      paneId,
    );
  }
}
