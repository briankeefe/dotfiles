...
import { realpath, mkdir, writeFile, rmdir } from "node:fs/promises";
...
if (args.includes("--help")) {
  ...
  const name = await (async () => {
    const lockPath = path.join(sessionDir, "launch.lock");
    try {
      await mkdir(lockPath, { mode: 0o700 });
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === "EEXIST")
        throw new Error(
          `Another Foreman launch is in progress: ${lockPath}. If this lock is stale, confirm no launcher is running before manually removing that exact empty lock directory.`,
        );
      throw error;
    }
    try {
