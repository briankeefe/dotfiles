...
type Coordinator = {
  ...
  start(args: Assignment): Promise<Worker>;
  inspect(id: string): Promise<Agent>;
  read(id: string): Promise<unknown>;
  prompt(id: string, text: string, observedInputAt: string): Promise<void>;
...
export default function foremanExtension(pi: ExtensionAPI) {
  ...
  function coordinator(ctx: ExtensionContext, initial: State): Coordinator {
    ...
    async function observe(id: string) {
      ...
      try {
        ...
        const tokens = agent.tokens ?? {};
        const w = worker(id);
        if (tokens.foreman_pr) {
          const pr = prSchema.parse(JSON.parse(tokens.foreman_pr));
          if (Number.isSafeInteger(pr.number) && pr.url !== w.pr?.url)
            patch(id, { pr });
        }
    ...
    async function prompt(
      ...
      try {
        ...
        if (["paused", "blocked"].includes(agent.tokens?.foreman_status ?? ""))
          throw new Error(
    ...
    async function start(args: Assignment) {
      ...
      try {
        ...
        if ((await git(repo, "rev-parse", "--show-toplevel")) !== repo)
        ...
        await git(repo, "check-ref-format", "--branch", base);
        const localParent = parent && parent.repo === repo && !args.base;
        if (!localParent) await git(repo, "fetch", "origin", base);
        ...
        try {
          ...
          const socketPath = await ensureSession(w.sessionName, cwd);
          patch(id, { socketPath });
          const launched = await launch(worker(id));
    ...
    async function launch(w: Worker, name = w.name) {
  ...
  pi.registerTool({
    ...
    label: "Foreman",
    description:
      "Dispatch user-requested tasks; inspect/nudge independent OMP workers; notify for decisions. Requires /foreman. No automatic restarts, approvals, ready transitions or merges.",
    parameters: z.object({
      op: z.enum(["list", "start", "read", "prompt", "notify"]),
    ...
    async execute(_id, params, _signal, _update, ctx) {
      ...
      const result = await (async () => {
        ...
        if (params.op === "read") return controller.read(params.id);
