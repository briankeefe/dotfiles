...
Present the exact comments/body and proposed event (`COMMENT`, `REQUEST_CHANGES`, `APPROVE`, or host equivalent). Obtain approval before posting unless the user's current request already authorizes that review publication. A local recommendation is not a published approval.
